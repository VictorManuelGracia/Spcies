%% cons_MPCT_x1_free_ADMM_semiband_C
%
% Generates the constructor for C of the MPCT_x1_free formulation solved using ADMM.
%
...
%              - .tol_p: Exit (primal) tolerance of the solver.
%              - .tol_d: Exit (dual) tolerance of the solver.
...
function constructor = cons_MPCT_x1_free_ADMM_semiband_C(recipe)
    ...
    vars = MPCT.compute_MPCT_x1_free_ADMM_ingredients(recipe.controller, recipe.options);
    ...
    if vars.rho_is_scalar
        ...
        defCell = add_line(defCell, 'alpha_rho_i', vars.alpha_rho_i, 1, precision, 'define');
        ...
        constCell = add_line(constCell, 'alpha_rho_i', vars.alpha_rho_i, 1, precision, var_options);
    ...
    if vars.rho_is_scalar
        constCell = add_line(constCell, 'U_tilde_ini', vars.U_tilde_ini, 1, precision, var_options);
        constCell = add_line(constCell, 'U_tilde_mid', vars.U_tilde_mid, 1, precision, var_options);
        constCell = add_line(constCell, 'U_tilde_final', vars.U_tilde_final, 1, precision, var_options);
    else
    ...
    constructor.files.code.blocks = {'$START$', C_code.get_generic_solver_struct;...
                                     '$INSERT_SOLVER$', [this_path '/code_MPCT_x1_free_ADMM_C.c']};

    % .h file
    constructor = constructor.new_empty_file('header',recipe.options,'h');
    constructor.files.header.blocks = {'$START$', [this_path '/header_MPCT_x1_free_ADMM_C.h']};
