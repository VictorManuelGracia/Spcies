...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    if(nrhs != 3){
        mexErrMsgIdAndTxt("Spcies:MPCT_x1_free_ADMM:nrhs:number",
                          "Three inputs are required");
    }

    // Check number of outputs
    if(nlhs == 0){
        mexErrMsgIdAndTxt("Spcies:MPCT_x1_free_ADMM:nlhs:number",
    ...
    if( !mxIsDouble(prhs[0]) || mxGetNumberOfElements(prhs[0]) != nn_ ){
        mexErrMsgIdAndTxt("Spcies:MPCT_x1_free_ADMM:nrhs:x0",
    ...
    if( !mxIsDouble(prhs[1]) || mxGetNumberOfElements(prhs[1]) != nn_ ){
        mexErrMsgIdAndTxt("Spcies:MPCT_x1_free_ADMM:nrhs:xr",
    ...
    if( !mxIsDouble(prhs[2]) || mxGetNumberOfElements(prhs[2]) != mm_ ){
        mexErrMsgIdAndTxt("Spcies:MPCT_x1_free_ADMM:nrhs:ur",
    ...
    z_pt = mxCreateDoubleMatrix((NN_+1)*nm_+nn_, 1, mxREAL);
    v_pt = mxCreateDoubleMatrix((NN_+1)*nm_+nn_, 1, mxREAL);
    lambda_pt = mxCreateDoubleMatrix((NN_+1)*nm_+nn_, 1, mxREAL);
    ...
    // Call solver
    MPCT_x1_free_ADMM(x0, xr, ur, u_opt, &k, &e_flag, &sol);
