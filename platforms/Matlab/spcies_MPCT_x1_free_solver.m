%% spcies_MPCT_x1_free_solver -  Solver for the MPCT formulation with double-first predicted state using the ADMM algorithm
%
% This is a non-sparse solver for a modified MPC for Tracking formulation with 
% double-first predicted state from the Spcies toolbox.
%
% This solver considers two different first states in the predicted sequence of states, and penalizes the difference between them 
% using an exact penalty function. One of them is based on the current state of the system, while the other sets the initial state
% for the rest of the sequence of predicted states.
% It also uses the Woodbury Matrix Identity to efficiently compute the iterate z^{k+1} of ADMM when it is applied to the MPC for Tracking formulation.
...
%
% TODO: Change alpha and beta penalization terms from options to necessary input of the solver.


function [u, k, e_flag, Hist] = spcies_MPCT_x1_free_solver(x0, xr, ur, varargin)
    import MPCT.compute_MPCT_x1_free_ADMM_ingredients
    ...
    def_options = MPCT.def_options_MPCT_x1_free_ADMM_semiband();

    %% Parser
    par = inputParser;
    par.CaseSensitive = false;
    par.FunctionName = 'spcies_MPCT_x1_free_solver';
    ...
    options = Spcies_options('formulation', 'MPCT_x1_free', 'method', 'ADMM', 'submethod', 'semiband', 'options', options);
    ...
    var = compute_MPCT_x1_free_ADMM_ingredients(controller,options);
    N = var.N; % Prediction horizon
    ...
    k = 0;
    z = zeros((N+2)*n+(N+1)*m,1);
    v = zeros((N+2)*n+(N+1)*m,1);
    v_old = zeros((N+2)*n+(N+1)*m,1); % Value of v in the previuos iteration
    lambda = zeros((N+2)*n+(N+1)*m,1);
    ...
    if genHist > 1
        hZ = zeros((N+2)*n+(N+1)*m,options.solver.k_max);
        hV = zeros((N+2)*n+(N+1)*m,options.solver.k_max);
        hLambda = zeros((N+2)*n+(N+1)*m,options.solver.k_max);
    end
    ...
    while ~done
        ...
        % Compute xi from eq. (9a). TODO: Check if the number of equation is correct
        
        % Obtaining z1_a = var.Gamma_hat\p
        % The following code solves a banded-diagonal system of equations.
        % 1st banded-diagonal system
        if isscalar(var.rho)
            z1_a(1:n,1) = var.Q_rho_i*p(1:n);
            z1_a(n+1:n+m,1) = var.R_rho_i*p(n+1:n+m);
            z1_a(n+m+1:n+m+n) = var.rho_i*p(n+m+1:n+m+n);
            ...
            z1_a(1:n,1) = var.Q_rho_i(:,:,1)*p(1:n);
            z1_a(n+1:n+m,1) = var.R_rho_i(:,:,1)*p(n+1:n+m);
            z1_a(n+m+1:n+m+n) = var.rho_i(n+m+1:n+m+n,1).*p(n+m+1:n+m+n); % OK
            for i = n+m+n+1:n+m:(N+1)*n+N*m
        ...
        z1_a((N+1)*n+N*m+1:(N+1)*n+N*m+n) = var.T_rho_i*p((N+1)*n+N*m+1:(N+1)*n+N*m+n);
        z1_a((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m) = var.S_rho_i*p((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m);
        ...
        if isscalar(var.rho)
            z3_a(1:n,1) = var.Q_rho_i*vec(1:n);
            z3_a(n+1:n+m,1) = var.R_rho_i*vec(n+1:n+m);
            z3_a(n+m+1:n+m+n) = var.rho_i*vec(n+m+1:n+m+n);
            for i = n+m+n+1:n+m:(N+1)*n+N*m
            ...
            z3_a(1:n,1) = var.Q_rho_i(:,:,1)*vec(1:n);
            z3_a(n+1:n+m,1) = var.R_rho_i(:,:,1)*vec(n+1:n+m);
            z3_a(n+m+1:n+m+n) = diag(var.rho_i(n+m+1:n+m+n,1))*vec(n+m+1:n+m+n); % OK
            for i = n+m+n+1:n+m:(N+1)*n+N*m
        ...
        z3_a((N+1)*n+N*m+1:(N+1)*n+N*m+n) = var.T_rho_i*vec((N+1)*n+N*m+1:(N+1)*n+N*m+n);
        z3_a((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m) = var.S_rho_i*vec((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m);
        % End of code for banded-diagonal system of equations.

        % Computation of xi
        xi = z1_a - z3_a;

        % Compute mu from eq. (9b). TODO: Check if the number of equation is correct
        ...
        % Compute z^{k+1} from eq. (9c). TODO: Check if the number of equation is correct
        ...
        if isscalar(var.rho)
            z1_c(1:n,1) = var.Q_rho_i*vec(1:n);
            z1_c(n+1:n+m,1) = var.R_rho_i*vec(n+1:n+m);
            z1_c(n+m+1:n+m+n) = var.rho_i*vec(n+m+1:n+m+n);
            for i = n+m+n+1:n+m:(N+1)*n+N*m
            ...
            z1_c(1:n,1) = var.Q_rho_i(:,:,1)*vec(1:n);
            z1_c(n+1:n+m,1) = var.R_rho_i(:,:,1)*vec(n+1:n+m);
            z1_c(n+m+1:n+m+n) = diag(var.rho_i(n+m+1:n+m+n,1))*vec(n+m+1:n+m+n); % OK
            for i = n+m+n+1:n+m:(N+1)*n+N*m
        ...
        z1_c((N+1)*n+N*m+1:(N+1)*n+N*m+n) = var.T_rho_i*vec((N+1)*n+N*m+1:(N+1)*n+N*m+n);
        z1_c((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m) = var.S_rho_i*vec((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m);
        % End of code for banded-diagonal system of equations.
        ...
        if isscalar(var.rho)
            z3_c(1:n,1) = var.Q_rho_i*vec(1:n);
            z3_c(n+1:n+m,1) = var.R_rho_i*vec(n+1:n+m);
            z3_c(n+m+1:n+m+n) = var.rho_i*vec(n+m+1:n+m+n);
            for i = n+m+n+1:n+m:(N+1)*n+N*m
            ...
            z3_c(1:n,1) = var.Q_rho_i(:,:,1)*vec(1:n);
            z3_c(n+1:n+m,1) = var.R_rho_i(:,:,1)*vec(n+1:n+m);
            z3_c(n+m+1:n+m+n) = diag(var.rho_i(n+m+1:n+m+n,1))*vec(n+m+1:n+m+n); % OK
            for i = n+m+n+1:n+m:(N+1)*n+N*m
        ...
        z3_c((N+1)*n+N*m+1:(N+1)*n+N*m+n) = var.T_rho_i*vec((N+1)*n+N*m+1:(N+1)*n+N*m+n);
        z3_c((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m) = var.S_rho_i*vec((N+1)*n+N*m+n+1:(N+1)*n+N*m+n+m);
        % End of code for banded-diagonal system of equations.
        ...
        % Elements related to u_0. Note that x_0 is unconstrained
        v(n+1:n+m) = min(max(v(n+1:n+m),var.LB(n+1:n+m)),var.UB(n+1:n+m)); % Box constraints of u_0

        % Elements related to (\hat{x}_1,x_1)
        if isscalar(var.rho)
            for i = n+m+1:n+m+n
                [v(i),v(n+i)] = solve_abs_maxQP(v(i),v(n+i),var.alpha_rho_i,var.beta_rho_i,var.LB(i-n-m),var.UB(i-n-m),var.LB(i-n-m),var.UB(i-n-m));
            end
        else
            for i = n+m+1:n+m+n
                [v(i),v(n+i)] = solve_abs_maxQP(v(i),v(n+i),var.alpha_rho_i(i),var.beta_rho_i(i),var.LB(i-n-m),var.UB(i-n-m),var.LB(i-n-m),var.UB(i-n-m));
                % TODO: There are two possible alpha_rho_i values since we have as many 
                % rho as decision variables, so there is one alpha_rho_i vector for \hat{x}_1, 
                % and one alpha_rho_i vector for x_1. Maybe we can do the average? 
            end
        end

        % Elements related to the rest of the predicted sequence:
        % (u_1, x_2, u_2, ..., x_{N-1}, u_{N-1}, x_s, u_s)
        v(n+m+n+n+1:n+m+n+n+m) = min(max(v(n+m+n+n+1:n+m+n+n+m),var.LB(n+1:n+m)),var.UB(n+1:n+m)); % Box constraints for u_1
        for i = n+m+n+n+m+1:n+m:(N+2)*n+(N+1)*m
            v(i:i+n+m-1) = min(max(v(i:i+n+m-1),var.LB),var.UB); % Box constraints for the rest of the predicted sequence
        end

        % Obtaining lambda^{k+1}
        if isscalar(var.rho)
